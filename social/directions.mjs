...
async function capturer(nav, port, { saison, meteo, ciel, zoom = 0, objets = ILE, mots }) {
...
const MAQUETTES = {
  ...
  detail: `async (c, im, t) => {
    ...
    poser(c, im, 0, 0, L, H, { zone:[0.08,0.00,0.94,1.00], ay:0.42 });
...
<canvas id="c" width="${L}" height="${H}"></canvas>`, { waitUntil: 'load' });
  ...
  /* Les outils et la maquette partent comme **sources**, et la page les
     assemble en une seule fonction : la maquette appelle `fondDe()` et
     `bloc()` par leur nom, comme si elles étaient dans le même fichier.
     Passer la maquette déjà compilée n'est pas possible — une fonction
     ne traverse pas la frontière du navigateur. */
  ...
  }, { b64s: pngs, mots: carte.mots, outils: OUTILS,
       maquette: MAQUETTES[carte.maquette], multi: !!carte.scenes });

  const jpg = await p.evaluate(() =>
    document.getElementById('c').toDataURL('image/jpeg', 0.93).split(',')[1]);
  fs.writeFileSync(OUT + carte.k + '.jpg', Buffer.from(jpg, 'base64'));
  await p.close();

  console.log(`  ${carte.k}  ${(fs.statSync(OUT + carte.k + '.jpg').size / 1024).toFixed(0)} ko`);
}
